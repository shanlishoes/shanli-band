...
export default function Home() {
  ...
  async function submit(e:React.FormEvent){
    ...
    setLoading(true)
    try { const r=await fetch('/api/attendance',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({staffId:selected.id,hours:value,edit:done.includes(selected.id)})}); const d=await r.json(); if(!r.ok) throw new Error(d.error||'ثبت انجام نشد');
  ...
  return <main className="shell"><div className="topbar"><div className="brand"><div className="logo">✓</div><div><b>کارکردینو</b><small>ثبت کارکرد روزانه</small></div></div><div className="date"><span className="dot"/> امروز<br/><b>{today}</b></div></div>
    <section className="hero"><div><div className="eyebrow">پنل پرسنل</div><h1>کارکرد امروزت را ثبت کن</h1><p>نام خودت را انتخاب کن و تعداد کارکرد امروز را وارد کن.</p></div><div className="heroShape">◷</div></section>
    {!selected ? <section><h2>چه کسی هستی؟</h2><div className="people">{staff.map(s=><button className={'person '+(done.includes(s.id)?'disabled':'')} key={s.id} onClick={()=>setSelected(s)}><span style={{background:s.color}}>{s.initials}</span><strong>{s.name}</strong>{done.includes(s.id)?<em>ویرایش ثبت ✓</em>:<i>انتخاب ›</i>}</button>)}</div><div className="info">ⓘ هر نفر فقط یک‌بار در روز می‌تواند کارکرد خود را ثبت کند.</div></section> : <section className="card formCard"><button className="back" onClick={()=>setSelected(null)}>← تغییر نام</button><div className="selected"><span style={{background:selected.color}}>{selected.initials}</span><div><small>در حال ثبت برای</small><h2>{selected.name}</h2></div><label>امروز<br/><b>{today}</b></label></div><form onSubmit={submit}><label className="field">تعداد کارکرد امروز <span>ضروری</span><div className="inputWrap"><input autoFocus type="number" min="0" step="1" placeholder="مثلاً ۸" value={hours} onChange={e=>setHours(e.target.value)}/><b>عدد</b></div></label>{error&&<div className="error">⚠ {error}</div>}<button className="primary" disabled={loading}>{loading?'در حال ثبت...':done.includes(selected?.id||'')?'ذخیره اصلاحات امروز  ✓':'ثبت کارکرد امروز  ✓'}</button></form></section>}
